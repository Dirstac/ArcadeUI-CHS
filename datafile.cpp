...
std::string load_driver_geninfo(const game_driver *drv, int drvindex)
{
	...
	if (BIT(cache, 6))
		buffer.append("此游戏无法正常运行。\n");

	if (BIT(cache, 22))
		buffer.append("此游戏有保护措施，未完全模拟。\n");

	if (BIT(cache, 18))
		buffer.append("视频模拟不是100%准确。\n");

	if (BIT(cache, 21))
		buffer.append("颜色完全错误。\n");

	if (BIT(cache, 20))
		buffer.append("颜色不是100%准确。\n");

	if (BIT(cache, 17))
		buffer.append("此游戏缺少声音。\n");

	if (BIT(cache, 16))
		buffer.append("声音模拟不是100%准确。\n");

	if (BIT(cache, 7))
		buffer.append("支持即时存档\n");
	else
		buffer.append("不支持即时存档。\n");

	if (BIT(cache, 14))
		buffer.append("此游戏包含机械设备。\n");

	if (BIT(cache, 15))
		buffer.append("此游戏未完成。\n");

	if (BIT(cache, 13))
		buffer.append("此游戏没有声音硬件。\n");
	...
	snprintf(name, std::size(name), "%s", GetDescriptionByIndex(drvindex, GetUsechineseList()));	//中文名称
	buffer.append(name);
	...
	if (has_sound)
	{
		...
		if(channels == 1)
			snprintf(name, std::size(name), "%d 个声道\n", channels);
		else
			snprintf(name, std::size(name), "%d 个声道\n", channels);
	...
	if (!is_bios)
	{
		...
		buffer.append(GetDescriptionByIndex(g, GetUsechineseList()));	//中文名称
		buffer.append("\n\n克隆:\n");
		...
		for (int i = 0; i < driver_list::total(); i++)
		{
			if (!strcmp (drv->name, driver_list::driver(i).parent))
			{
				...
				buffer.append(GetDescriptionByIndex(i, GetUsechineseList()));	//中文名称
				buffer.append("\n");
			}
		}
	}
	...
	for (int i = 0; i < driver_list::total(); i++)
	{
		...
		if ((strcmp(source_file, t1.c_str())==0) && !(DriverIsBios(i)))
			...
			buffer.append(GetDescriptionByIndex(i, GetUsechineseList())).append("\n");	//中文名称
	}
