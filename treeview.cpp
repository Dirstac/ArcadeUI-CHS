...
extern const FOLDERDATA g_folderData[] =
{
	...
	{"全部游戏",       "allgames",          FOLDER_ALLGAMES,     IDI_FOLDER_ALLGAMES,      0,             0,            0, NULL,                       NULL,                    true },
	{"拥有游戏",       "available",         FOLDER_AVAILABLE,    IDI_FOLDER_AVAILABLE,     F_AVAILABLE,   0,            0, NULL,                       FilterAvailable,         true },
	{"未拥有游戏",     "unavailable",       FOLDER_UNAVAILABLE,  IDI_FOLDER_UNAVAILABLE,   0,             F_AVAILABLE,  0, NULL,                       FilterAvailable,         false },
	{"可以运行",         "working",           FOLDER_WORKING,      IDI_FOLDER_WORKING,       F_WORKING,     F_NONWORKING, 0, NULL,                       DriverIsBroken,          false },
	{"无法运行",     "nonworking",        FOLDER_NONWORKING,   IDI_FOLDER_NONWORKING,    F_NONWORKING,  F_WORKING,    0, NULL,                       DriverIsBroken,          true },
	{"原作",         "originals",         FOLDER_ORIGINAL,     IDI_FOLDER_ORIGINALS,     F_ORIGINALS,   F_CLONES,     0, NULL,                       DriverIsClone,           false },
	{"克隆游戏",          "clones",            FOLDER_CLONES,       IDI_FOLDER_CLONES,        F_CLONES,      F_ORIGINALS,  0, NULL,                       DriverIsClone,           true },
	{"年份",            "year",              FOLDER_YEAR,         IDI_FOLDER_YEAR,          0,             0,            0, CreateYearFolders },
	{"生产厂商",    "manufacturer",      FOLDER_MANUFACTURER, IDI_FOLDER_MANUFACTURER,  0,             0,            0, CreateManufacturerFolders },
	...
	{"游戏驱动",          "source",            FOLDER_SOURCE,       IDI_FOLDER_SOURCE,        0,             0,            0, CreateSourceFolders },
	{"转储状态",  "dumping",           FOLDER_DUMPING,      IDI_FOLDER_DUMP,          0,             0,            1, CreateDumpingFolders },
	{"不完美",       "imperfect",         FOLDER_DEFICIENCY,   IDI_FOLDER_IMPERFECT,     0,             0,            0, CreateDeficiencyFolders },
	{"光枪",        "lightgun",          FOLDER_LIGHTGUN,     IDI_FOLDER_LIGHTGUN,      0,             0,            0, NULL,                       DriverUsesLightGun,      true },
	{"轨迹球",       "trackball",         FOLDER_TRACKBALL,    IDI_FOLDER_TRACKBALL,     0,             0,            0, NULL,                       DriverUsesTrackball,     true },
	{"机械游戏",      "mechanical",        FOLDER_MECHANICAL,   IDI_FOLDER_MECHANICAL,    0,             0,            0, NULL,                       DriverIsMechanical,      true },
	//{"非机械游戏",  "nonmechanical",     FOLDER_NONMECHANICAL,IDI_FOLDER,               0,             0,            0, NULL,                       DriverIsMechanical,      FALSE },
	{"刷新",         "refresh",           FOLDER_FPS,          IDI_FOLDER_FPS,           0,             0,            1, CreateFPSFolders },
	{"分辨率",      "resolution",        FOLDER_RESOLUTION,   IDI_FOLDER_RESOL,         0,             0,            1, CreateResolutionFolders },
	{"采样文件",         "samples",           FOLDER_SAMPLES,      IDI_FOLDER_SAMPLES,       0,             0,            0, NULL,                       DriverUsesSamples,       true },
	{"支持存档",       "savestate",         FOLDER_SAVESTATE,    IDI_FOLDER_SAVESTATE,     0,             0,            0, CreateSaveStateFolders },
	{"屏幕",         "screens",           FOLDER_SCREENS,      IDI_FOLDER_MONITOR,       0,             0,            0, CreateScreenFolders },
	{"音频",           "sound",             FOLDER_SOUND,        IDI_FOLDER_SOUND,         0,             0,            1, CreateSoundFolders },
	//{"立体声",          "stereo",            FOLDER_STEREO,       IDI_FOLDER,               0,             0,            0, NULL,                       DriverIsStereo,          TRUE },
	{"点阵画面",          "raster",            FOLDER_RASTER,       IDI_FOLDER_RASTER,        F_RASTER,      F_VECTOR,     0, NULL,                       DriverIsVector,          false },
	{"矢量画面",          "vector",            FOLDER_VECTOR,       IDI_FOLDER_VECTOR,        F_VECTOR,      F_RASTER,     0, NULL,                       DriverIsVector,          true },
	{"垂直",        "vertical",          FOLDER_VERTICAL,     IDI_FOLDER_VERTICAL,      F_VERTICAL,    F_HORIZONTAL, 0, NULL,                       DriverIsVertical,        true },
	{"水平",      "horizontal",        FOLDER_HORIZONTAL,   IDI_FOLDER_HORIZONTAL,    F_HORIZONTAL,  F_VERTICAL,   0, NULL,                       DriverIsVertical,        false },
	{ NULL }
};
...
// =========================  搜索 ========================>>>
    if (strlen(GetSearchText()) && _stricmp(GetSearchText(), SEARCH_PROMPT))
    {
        ...
        if (MyStrStrI(GetDescriptionByIndex(nGame, GetUsechineseList()), search_text) != NULL)
        {

        }

        else if (MyStrStrI(GetGameNameByIndex(nGame, GetUsechineseList()), search_text) != NULL)
        {

        }

        else if (MyStrStrI(GetGameManufactureByIndex(nGame, GetUsechineseList()), search_text) != NULL)
        {
...
static void CreateDeficiencyFolders(int parent_index)
{
	...
	lpWrongCol = NewFolder("颜色错误", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpImpCol = NewFolder("不完美的颜色", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpImpGraph = NewFolder("不完美的图形", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpMissSnd = NewFolder("缺少声音", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpImpSnd = NewFolder("不完美的声音", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpIncomplete = NewFolder("不完整的原型", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpNoSndHw = NewFolder("无声音硬件", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
...
static void CreateSaveStateFolders(int parent_index)
{
	...
	lpSupported = NewFolder("支持", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpUnsupported = NewFolder("未支持", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
...
void CreateDumpingFoldersIni(int parent_index)
{
	...
	SetAllBits(lpFolder->m_lpGameBits,false);
	// create our two subfolders
	lpNoDump = NewFolder("未转储", next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpBadDump = NewFolder("不良转储", next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpGoodDump = NewFolder("完美转储", next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	AddFolder(lpNoDump);
	AddFolder(lpBadDump);
...
static bool CreateTreeIcons(void)
{
	...
	for (int i = 0; i < ICON_MAX; i++)
	{
		...
		{
			ErrorMessageBox("在普通文件夹上创建图标时出错， %i %i", i, hIcon != NULL);
			return false;
		}
	}
	...
	for (int i = 0; i < numExtraIcons; i++)
	{
		...
		if (ImageList_AddIcon(hTreeSmall, hIcon) == -1)
		{
			ErrorMessageBox("在额外文件夹上创建图标时出错，%i %i", i, hIcon != NULL);
			return false;
		}
	}
	...
	{
		ErrorMessageBox("在额外文件夹上创建图标时出错，%i %i", ImageList_GetImageCount(hTreeSmall), numIcons);
		return false;
	}
	...
	{
		ErrorMessageBox("图标列表错误——图像太少。  %i < %i", ImageList_GetImageCount(hTreeSmall), ICON_MAX);
		return false;
	}
...
bool TryAddExtraFolderAndChildren(int parent_index)
{
	...
	while (fgets(readbuf, 256, f))
	{
		...
		else if (current_id != -1)
		{
			...
			if (lpTemp == NULL)
			{
				ErrorMessageBox("解析 %s 时出错：缺少 [folder name] 或 [ROOT_FOLDER]", fname);
				current_id = lpFolder->m_nFolderId;
				lpTemp = lpFolder;
			}
...
bool TryRenameCustomFolder(LPTREEFOLDER lpFolder, const char *new_name)
{
	...
	if (retval)
	{
		...
		ErrorMessageBox("将自定义文件 %s 重命名为 %s 时出错", filename, new_filename);
...
void AddToCustomFolder(LPTREEFOLDER lpFolder, int driver_index)
{
	if ((lpFolder->m_dwFlags & F_CUSTOM) == 0)
	{
		ErrorMessageBox("无法将游戏添加到非自定义文件夹");
		return;
	}
...
void RemoveFromCustomFolder(LPTREEFOLDER lpFolder, int driver_index)
{
	if ((lpFolder->m_dwFlags & F_CUSTOM) == 0)
	{
		ErrorMessageBox("无法从非自定义文件夹中删除游戏");
		return;
	}
...
bool TrySaveExtraFolder(LPTREEFOLDER lpFolder)
{
	...
	if (error)
		ErrorMessageBox("保存自定义文件 %s 时出错", fname);
