...
//================== 缘来是你 ==========================>>>
	{ OPTION_IPSPATH,                                    "ips",       core_options::option_type::MULTIPATH,  "IPS 补丁文件的路径" },
	{ OPTION_IPS,                                        nullptr,     core_options::option_type::STRING,     "IPS 路径名称" },
	{ OPTION_SKIP_CRC_CHECK,                             "0",         core_options::option_type::BOOLEAN,    "跳过 CRC 和 HASH 校验" },	
	{ OPTION_PGM2_MEMCARD_HACK, 						 "1", 		  core_options::option_type::BOOLEAN, 	 "启用 PGM2 免插卡" },
//======================================================>>>
	...
	{ OPTION_AUTOSAVE,                                   "0",         core_options::option_type::BOOLEAN,    "对于支持的系统，在启动时自动恢复存档并在退出时自动保存" },
	{ OPTION_REWIND,                                     "0",         core_options::option_type::BOOLEAN,    "启用游戏回退功能" },
	{ OPTION_REWIND_CAPACITY "(1-2048)",                 "100",       core_options::option_type::INTEGER,    "倒带缓冲区大小（以兆字节为单位）" },
	...
	{ OPTION_EXIT_AFTER_PLAYBACK,                        "0",         core_options::option_type::BOOLEAN,    "播放结束关闭程序" },
	...
	{ OPTION_SNAPBILINEAR,                               "1",         core_options::option_type::BOOLEAN,    "指定截图/视频是否应用双线性过滤" },
	{ OPTION_STATENAME,                                  "%g",        core_options::option_type::STRING,     "override of the default state subfolder naming; %g == gamename" },
	{ OPTION_BURNIN,                                     "0",         core_options::option_type::BOOLEAN,    "为每个屏幕创建烧录快照" },

	// performance options
	{ nullptr,                                           nullptr,     core_options::option_type::HEADER,     "CORE PERFORMANCE OPTIONS" },
	{ OPTION_AUTOFRAMESKIP ";afs",                       "0",         core_options::option_type::BOOLEAN,    "启用自动跳帧调整以保持模拟速度" },
	{ OPTION_FRAMESKIP ";fs(0-10)",                      "0",         core_options::option_type::INTEGER,    "设置跳帧为固定值，0-10（自动跳帧上限）" },
	...
	{ OPTION_THROTTLE,                                   "1",         core_options::option_type::BOOLEAN,    "启用速度限制，以保持系统运行与实时同步" },
	{ OPTION_SYNCREFRESH ";srf",                         "0",         core_options::option_type::BOOLEAN,    "启用 VBLANK 限制游戏速度，而非游戏时间" },  // MAMEFX, Mamesick 2016-08-31
	{ OPTION_SLEEP,                                      "1",         core_options::option_type::BOOLEAN,    "启用睡眠，在空闲时释放系统资源给其他应用程序" },
	{ OPTION_SPEED "(0.1-100)",                         "1.0",       core_options::option_type::FLOAT,      "控制游戏速度，相对于实时； 数字越小越慢" },
	{ OPTION_REFRESHSPEED ";rs",                         "0",         core_options::option_type::BOOLEAN,    "自动调整模拟速度以保持模拟刷新率低于主机屏幕" },
	{ OPTION_LOWLATENCY ";lolat",                        "0",         core_options::option_type::BOOLEAN,    "在限制之前绘制新帧以减少输入延迟" },

	// render options
	{ nullptr,                                           nullptr,     core_options::option_type::HEADER,     "CORE RENDER OPTIONS" },
	{ OPTION_KEEPASPECT ";ka",                           "1",         core_options::option_type::BOOLEAN,    "当缩放填充输出屏幕/窗口时，保持纵横比" },
	...
	{ OPTION_ROTATE,                                     "1",         core_options::option_type::BOOLEAN,    "需要时根据游戏的方向旋转游戏画面" },
	{ OPTION_ROR,                                        "0",         core_options::option_type::BOOLEAN,    "顺时针旋转屏幕 90 度" },
	{ OPTION_ROL,                                        "0",         core_options::option_type::BOOLEAN,    "逆时针旋转屏幕 90 度" },
	{ OPTION_AUTOROR,                                    "0",         core_options::option_type::BOOLEAN,    "垂直画面自动顺时针旋转屏幕 90 度" },
	{ OPTION_AUTOROL,                                    "0",         core_options::option_type::BOOLEAN,    "垂直画面自动逆时针旋转屏幕 90 度" },
	{ OPTION_FLIPX,                                      "0",         core_options::option_type::BOOLEAN,    "左右翻转屏幕" },
	{ OPTION_FLIPY,                                      "0",         core_options::option_type::BOOLEAN,    "上下翻转屏幕" },

	// artwork options
	{ nullptr,                                           nullptr,     core_options::option_type::HEADER,     "CORE ARTWORK OPTIONS" },
	{ OPTION_ARTWORK_CROP ";artcrop",                    "0",         core_options::option_type::BOOLEAN,    "裁剪装饰图以适应屏幕" },
	{ OPTION_FALLBACK_ARTWORK,                           nullptr,     core_options::option_type::STRING,     "若未定义外部装饰图或内部驱动程序布局，则使用备用装饰图" },
	{ OPTION_OVERRIDE_ARTWORK,                           nullptr,     core_options::option_type::STRING,     "覆盖外部装饰图和内部驱动程序布局的装饰图" },
	...
	{ OPTION_COIN_LOCKOUT ";coinlock",                   "1",         core_options::option_type::BOOLEAN,    "若投币锁定输出处于激活状态，则忽略硬币输入" },
	{ OPTION_CTRLR,                                      nullptr,     core_options::option_type::STRING,     "preconfigure for specified controller" },
	{ OPTION_MOUSE,                                      "0",         core_options::option_type::BOOLEAN,    "启用鼠标输入" },
	{ OPTION_JOYSTICK ";joy",                            "1",         core_options::option_type::BOOLEAN,    "启用操纵杆输入" },
	{ OPTION_LIGHTGUN ";gun",                            "0",         core_options::option_type::BOOLEAN,    "启用光枪输入" },
	{ OPTION_MULTIKEYBOARD ";multikey",                  "0",         core_options::option_type::BOOLEAN,    "启用每个键盘设备的独立输入（如果有）" },
	{ OPTION_MULTIMOUSE,                                 "0",         core_options::option_type::BOOLEAN,    "启用每个鼠标设备的独立输入（如果有）" },
	{ OPTION_STEADYKEY ";steady",                        "0",         core_options::option_type::BOOLEAN,    "启用多键同时输入支持" },
	{ OPTION_UI_ACTIVE,                                  "0",         core_options::option_type::BOOLEAN,    "在模拟键盘上启用用户界面（如果有）" },
	{ OPTION_JOYSTICK_MAP ";joymap",                     "auto",      core_options::option_type::STRING,     "explicit joystick map, or auto to auto-select" },
	{ OPTION_JOYSTICK_DEADZONE ";joy_deadzone;jdz(0.00-1)",      "0.15",       core_options::option_type::FLOAT,      "操纵杆中心盲区忽略范围 (0.0 - 1.0)" },
	{ OPTION_JOYSTICK_SATURATION ";joy_saturation;jsat(0.00-1)", "0.85",      core_options::option_type::FLOAT,      "操纵杆末端饱和度忽略范围 (0.0 - 1.0)" },
	{ OPTION_JOYSTICK_THRESHOLD ";joy_threshold;jthresh(0.00-1)", "0.3",  core_options::option_type::FLOAT,  "将操纵杆视为激活开关的阈值 (0.0 - 1.0)" },
	{ OPTION_NATURAL_KEYBOARD ";nat",                    "0",         core_options::option_type::BOOLEAN,    "指定是否使用自然键盘" },
	{ OPTION_JOYSTICK_CONTRADICTORY ";joy_contradictory","0",         core_options::option_type::BOOLEAN,    "同时启用相反方向数字操纵杆输入" },
	{ OPTION_COIN_IMPULSE,                               "0",         core_options::option_type::INTEGER,    "设置投币脉冲时间（n<0 禁用脉冲，n==0 驱动控制，0<n 设置时间 n）" },

	// input autoenable options
	{ nullptr,                                           nullptr,     core_options::option_type::HEADER,     "CORE INPUT AUTOMATIC ENABLE OPTIONS" },
	{ OPTION_PADDLE_DEVICE ";paddle",                    "keyboard",  core_options::option_type::STRING,     "若存在划桨控制，则启用 (none|keyboard|mouse|lightgun|joystick)" },
	{ OPTION_ADSTICK_DEVICE ";adstick",                  "keyboard",  core_options::option_type::STRING,     "若存在模拟操纵杆控件，则启用 (none|keyboard|mouse|lightgun|joystick)" },
	{ OPTION_PEDAL_DEVICE ";pedal",                      "keyboard",  core_options::option_type::STRING,     "若存在踏板控制，则启用 (none|keyboard|mouse|lightgun|joystick)" },
	{ OPTION_DIAL_DEVICE ";dial",                        "keyboard",  core_options::option_type::STRING,     "若存在方向盘控制，则启用 (none|keyboard|mouse|lightgun|joystick)" },
	{ OPTION_TRACKBALL_DEVICE ";trackball",              "keyboard",  core_options::option_type::STRING,     "若存在轨迹球控制，则启用 (none|keyboard|mouse|lightgun|joystick)" },
	{ OPTION_LIGHTGUN_DEVICE,                            "keyboard",  core_options::option_type::STRING,     "若存在光枪控制，则启用 (none|keyboard|mouse|lightgun|joystick)" },
	{ OPTION_POSITIONAL_DEVICE,                          "keyboard",  core_options::option_type::STRING,     "若存在定位器控制，则启用 (none|keyboard|mouse|lightgun|joystick)" },
	{ OPTION_MOUSE_DEVICE,                               "mouse",     core_options::option_type::STRING,     "若存在鼠标控制，则启用 (none|keyboard|mouse|lightgun|joystick)" },
	...
	{ OPTION_CHEAT ";c",                                 "1",         core_options::option_type::BOOLEAN,    "启用作弊子系统" },
	{ OPTION_SKIP_GAMEINFO,                              "0",         core_options::option_type::BOOLEAN,    "在启动时跳过显示系统信息屏幕" },
	...
	{ OPTION_CONFIRM_QUIT,                               "0",         core_options::option_type::BOOLEAN,    "退出前询问" },
	{ OPTION_UI_MOUSE,                                   "1",         core_options::option_type::BOOLEAN,    "显示用户界面鼠标光标" },
	{ OPTION_LANGUAGE ";lang",                           "Chinese_Simplified",          core_options::option_type::STRING,     "set UI display language" },
	{ OPTION_NVRAM_SAVE ";nvwrite",                      "1",         core_options::option_type::BOOLEAN,    "save NVRAM data on exit" },
	{ "chinesename",                                     nullptr,     core_options::option_type::STRING,     "Chinese name for the game (used in window title)" },	//缘来是你
...
//=============================== 缘来是你 ==================================>>>
	...
	{ OPTION_60FPS,                                      "0",         core_options::option_type::BOOLEAN,     "固定游戏帧为 60 帧/秒" },
//==========================================================================>>>
