...
static bool FastRomSetFound(int index);	//缘来是你
...
intptr_t CALLBACK AuditWindowProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
			...
			winui_set_window_text_utf8(hAudit, "正在校验游戏... 请稍等...");
...
intptr_t CALLBACK GameAuditDialogProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			snprintf(tmp, std::size(tmp), "\"%s\"的校验结果", GetDriverGameName(rom_index));
...
static void ProcessNextRom(void)
{
	...
	{
		winui_set_window_text_utf8(hAudit, "校验已完成");
		DetailsPrintf("完成校验。\n");
		winui_set_window_text_utf8(GetDlgItem(hAudit, IDCANCEL), "关闭");
		rom_index = -1;
	}
}
