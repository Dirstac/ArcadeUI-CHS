...
} g_ComboBoxHistoryTab[] = 
{
	...
	{ TEXT("首领"),				TAB_BOSSES },
	{ TEXT("结局"),				TAB_ENDS },
	{ TEXT("游戏结束"),		TAB_GAMEOVER },
	{ TEXT("商标"),				TAB_LOGO },
	{ TEXT("装饰图"),			TAB_ARTWORK },
	{ TEXT("广告图"),			TAB_FLYER },
	{ TEXT("框体图"),			TAB_CABINET },
	{ TEXT("贴图"),			TAB_MARQUEE },
	{ TEXT("机台"),	TAB_CONTROL_PANEL },
	{ TEXT("电路图"),				TAB_PCB },
	{ TEXT("全部"),				TAB_ALL },
	{ TEXT("无"),				TAB_NONE }
};
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				case IDOK:
				{
					...
					if (resetFilters || resetGames || resetUI || resetDefaults || resetInternalUI)
					{
						...
						if (resetInternalUI)
							strcat(temp, "内部 MAME 接口选项\n");

						if (resetGames)
							strcat(temp, "个人游戏选项\n");

						if (resetFilters)
							strcat(temp, "自定义文件夹过滤器\n");

						if (resetUI)
						{
							strcat(temp, "用户界面设置\n\n");
							strcat(temp, "重置用户界面选项\n");
							strcat(temp, "需要退出 ");
						...
						if (winui_message_box_utf8(hDlg, temp, "恢复设置", MB_ICONQUESTION | MB_YESNO) == IDYES)
						{
							DestroyIcon(hIcon);
							DeleteObject(hBrush);
...
intptr_t CALLBACK InterfaceDialogProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			Button_SetCheck(GetDlgItem(hDlg, IDC_FAST_ROM_AUDIT), GetFastRomAudit());	//缘来是你
		...
		case WM_COMMAND:
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				...
				case IDOK:
				{
					...
					SetFastRomAudit(Button_GetCheck(GetDlgItem(hDlg, IDC_FAST_ROM_AUDIT)));	//缘来是你
	...
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			if (folder != NULL)
			{
				...
				snprintf(tmp, std::size(tmp), "%s 文件夹的过滤器", folder->m_lpTitle);
...
//=============================================================>>>
			...
			winui_set_window_text_utf8(GetDlgItem(hDlg, IDC_BUILD), "构建时间: " __DATE__" - " __TIME__"");
			snprintf(tmp, std::size(tmp), "版本: %s", long_build_version);
...
static void DisableVisualStylesInterface(HWND hDlg)
{
	...
	SetWindowTheme(GetDlgItem(hDlg, IDC_FAST_ROM_AUDIT), L" ", L" ");	//缘来是你
