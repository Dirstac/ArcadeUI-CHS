...
static wchar_t g_szPlayGameString[] = TEXT("运行 %s\tAlt+O");
static char g_szGameCountString[] = "%d 个游戏";
...
static void load_translation(emu_options &m_options)
{
	...
	{
		osd_printf_verbose("打开翻译文件 %s 时出错\n", name);
		name = "English";
		if (file.open(name + PATH_SEPARATOR "strings.mo"))
		{
			osd_printf_verbose("打开翻译文件 %s 时出错\n", name);
...
static void RunMAME(int nGameIndex, const play_options *playopts)
{
	...
	osd_printf_verbose("********** 开始 %s **********\n", name);
	osd_printf_info("********** 开始 %s **********\n", name);
...
//	unlink("verbose.log");
	...
	wcscpy(MameIcon.szInfo, TEXT("仍在运行...."));
...
static void Win32UI_init(void)
{
	...
	printf("Win32UI_init: 关于初始化选项\n");fflush(stdout);

	/* Init DirectInput */
	DirectInputInitialize();
	OptionsInit();

	printf("Win32UI_init: 已加载选项\n");fflush(stdout);

	if (RequiredDriverCache())
		winui_set_window_text_utf8(GetDlgItem(hSplash, IDC_PROGBAR), "正在构建文件夹结构...");
	else
		winui_set_window_text_utf8(GetDlgItem(hSplash, IDC_PROGBAR), "正在加载文件夹结构...");
...
//=======================================================================>>>
	...
	winui_set_window_text_utf8(GetDlgItem(hSplash, IDC_PROGBAR), "正在构建列表清单...");
	/* Initialize listview columns */
	InitListView();
	SendMessage(hProgress, PBM_SETPOS, 120, 0);
	winui_set_window_text_utf8(GetDlgItem(hSplash, IDC_PROGBAR), "正在初始化窗口...");
	ResetFonts();
	AdjustMetrics();
	UpdateScreenShot();
	...
	LoadGameListFromFile(game_total); // 缘来是你 USE_CLIST
...
static void Win32UI_exit(void)
{
	...
	SaveAllGameListToFile();	// 缘来是你	USE_CLIST
	...
	DeleteBitmap(hclist);// 缘来是你 USE_CLIST
...
//================================================>>>
		...
		case WM_CLOSE:
			if (GetExitDialog())
			{
				if (winui_message_box_utf8(hMain, "你确定要退出吗？", MAMEUINAME, MB_ICONQUESTION | MB_YESNO) == IDNO)
				{
					SetFocus(hWndList);
					return true;
				}
...
static bool GameCheck(void)
{
	...
	{
		SetStatusBarTextF(0, "游戏搜索 %d%% 已完成", percentage);
...
static void SetBatchDeleteMode(bool bEnable)
{
    ...
    if (bEnable)
    {
        ...
        SetStatusBarText(0, "批量删除模式已开启，请勾选要删除的游戏。按 ESC 退出。");
    }
    else
    {
        ListView_SetExtendedListViewStyle(hWndList, dwExStyle & ~LVS_EX_CHECKBOXES);
        SetStatusBarText(0, "批量删除模式已关闭");
    }
...
static char *GameInfoStatusBar(int driver_index)
{
	...
	if (DriverIsBroken(driver_index))
		return strcpy(status, "无法运行");
	else if (DriverIsImperfect(driver_index))
		return strcpy(status, "可以运行但有问题");
	else
		return strcpy(status, "可以运行");
}
...
// 缘来是你
//====================== USE_CLIST ============================>>>
	pText = GetDescriptionByIndex(nGame, GetUsechineseList());
	SetStatusBarText(0, pText);
	pName = GetGameNameByIndex(nGame,GetUsechineseList());
//==============================================================>>>
	SetStatusBarText(1, pName);
	SendMessage(hStatusBar, SB_SETICON, 1, (LPARAM)GetSelectedPickItemIconSmall());
...
static uintptr_t CALLBACK HookProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
			...
			if (bHookFont)
				winui_set_window_text_utf8(hDlg, "选择字体");
			else
				winui_set_window_text_utf8(hDlg, "选择颜色");
			break;
...
//==============================================================================>>>
		...
		{
			setlocale(LC_ALL,"Chinese"); //缘来是你 中文列表
...
//============================= 删除 ROM 文件 =======================>>>
		case ID_CONTEXT_DELETE_ROM:
		{
			...
			snprintf(msg, sizeof(msg), "确定要删除 ROM 文件吗？\n\n%s\n\n此操作不可恢复！", rompath);
			...
			if (SHFileOperationA(&shfo) == 0 && !shfo.fAnyOperationsAborted)
			{
				...
				winui_message_box_utf8(hWnd, "ROM 文件已删除。", MAMEUINAME, MB_ICONINFORMATION | MB_OK);
			}
			else
			{
				ErrorMessageBox("删除失败，请检查文件权限。");
			}
		...
		case ID_CONTEXT_DELETE_SELECTED_ROMS:
		{
			...
			if (nCheckedCount == 0)
			{
				ErrorMessageBox("请先勾选要删除的游戏（点击游戏前面的复选框）。");
				free(checkedIndices);
				break;
			}
			...
			if (nValidCount == 0)
			{
				ErrorMessageBox("没有找到可删除的 ROM 文件。");
				free(checkedIndices);
				free(roms);
				break;
			}
			...
			char msg[512];
			snprintf(msg, sizeof(msg), "确定要删除勾选的 %d 个 ROM 文件吗？\n\n此操作不可恢复！", nValidCount);
			if (winui_message_box_utf8(hWnd, msg, MAMEUINAME, MB_ICONWARNING | MB_YESNO) != IDYES)
			{
				free(checkedIndices);
				free(roms);
			...
			if (nSuccess > 0)
			{
				...
				char successMsg[256];
				snprintf(successMsg, sizeof(successMsg), "已删除 %d 个 ROM 文件。\n\n请按 F5 刷新列表。", nSuccess);
		...
		case ID_VIEW_ZIP:
		{
			...
			if (nGame >= 0)
			{
				...
				if (!found)    //zip file not found
					ErrorMessageBox("在 ROMS 路径中找不到 %s.zip：%s", GetDriverGameName(nGame), GetRomDirs());
			}
				...
				if (t_s)
				{
					if (PathFileExists(t_s))
					{
						...
						if (IsAuditResultYes(audit_result))
						{
							...
							ErrorMessageBox("游戏“%s”缺少rom ! \r\nM1FX 无法运行!", game);
					}
					else
						ErrorMessageBox("在当前文件夹中找不到M1FX.EXE !");
				}
			}
			SetFocus(hWndList);
			return true;
...
//	EnableMenuItem(GetMenu(hMain), ID_FILE_LOADSTATE, BIT(GetDriverCacheLower(nItem),7) ? MFS_ENABLED : MFS_GRAYED);
	EnableMenuItem(GetMenu(hMain), ID_FILE_LOADSTATE, (driver_list::driver(nItem).flags & MACHINE_SUPPORTS_SAVE) ? MFS_ENABLED : MFS_GRAYED);//缘来是你
}
...
bool CommonFileDialog(common_file_dialog_proc cfd, char *filename, int filetype, bool saving)
{
	...
	switch (filetype)
	{
		case FILETYPE_INPUT_FILES :
			...
			of.lpstrFilter = TEXT("录像 (*.inp,*.zip)\0*.inp;*.zip\0");
			of.lpstrDefExt = TEXT("inp");

			if (!saving)
				of.lpstrTitle  = TEXT("选择 INP 播放文件");
			else
				of.lpstrTitle  = TEXT("输入 INP 播放文件的名称");
		...
		case FILETYPE_SAVESTATE_FILES :
			path = GetStateDir();
			of.lpstrInitialDir = win_wstring_from_utf8(path);
			of.lpstrFilter = TEXT("游戏存档 (*.sta)\0*.sta;\0");
			of.lpstrDefExt = TEXT("sta");
			of.lpstrTitle  = TEXT("选择 STA 保存文件。");
			break;

		case FILETYPE_WAVE_FILES :
			path = GetAudioDir();
			of.lpstrInitialDir = win_wstring_from_utf8(path);
			of.lpstrFilter = TEXT("声音 (*.wav)\0*.wav;\0");
			of.lpstrDefExt = TEXT("wav");

			if (!saving)
				of.lpstrTitle  = TEXT("选择 WAV 音频文件");
			else
				of.lpstrTitle  = TEXT("输入 WAV 音频文件的名称");
		...
		case FILETYPE_MNG_FILES :
			path = GetVideoDir();
			of.lpstrInitialDir = win_wstring_from_utf8(path);
			of.lpstrFilter = TEXT("视频 (*.mng)\0*.mng;\0");
			of.lpstrDefExt = TEXT("mng");

			if (!saving)
				of.lpstrTitle  = TEXT("选择 MNG 图像文件");
			else
				of.lpstrTitle  = TEXT("输入 MNG 图像文件的名称");
		...
		case FILETYPE_AVI_FILES :
			path = GetVideoDir();
			of.lpstrInitialDir = win_wstring_from_utf8(path);
			of.lpstrFilter = TEXT("视频 (*.avi)\0*.avi;\0");
			of.lpstrDefExt = TEXT("avi");

			if (!saving)
				of.lpstrTitle  = TEXT("选择一个 AVI 视频文件");
			else
				of.lpstrTitle  = TEXT("输入 AVI 视频文件的名称");
		...
		case FILETYPE_EFFECT_FILES :
		{
			...
			of.lpstrFilter = TEXT("效果文件 (*.png)\0*.png;\0");
			of.lpstrDefExt = TEXT("png");
			of.lpstrTitle  = TEXT("选择叠加 PNG 效果文件");
			break;
		}
		...
		case FILETYPE_SHADER_FILES :
			...
			of.lpstrInitialDir = win_wstring_from_utf8(path);
			of.lpstrFilter = TEXT("着色器 (*.vsh)\0*.vsh;\0");
			of.lpstrDefExt = TEXT("vsh");
			of.lpstrTitle  = TEXT("选择 GLSL 着色器文件");
			break;
		...
		case FILETYPE_BGFX_FILES :
			...
			of.lpstrFilter = TEXT("效果 (*.json)\0*.json;\0");
			of.lpstrDefExt = TEXT("json");
			of.lpstrTitle  = TEXT("选择 BGFX 效果文件");
			break;

		case FILETYPE_LUASCRIPT_FILES :
			of.lpstrInitialDir = last_directory;
			of.lpstrFilter = TEXT("脚本 (*.lua)\0*.lua;\0");
			of.lpstrDefExt = TEXT("lua");
			of.lpstrTitle  = TEXT("选择 LUA 脚本文件");
			break;
	}
...
static void MamePlayBackGame(void)
{
	...
	*filename = 0;
	...
	if (CommonFileDialog(GetOpenFileName, filename, FILETYPE_INPUT_FILES, false))
	{
		...
		if (filerr)
		{
			ErrorMessageBox("无法将 '%s' 作为有效输入文件打开。", name);
			return;
		}
		...
		{
			ErrorMessageBox("输入文件已损坏或无效（缺少标头）。");
			return;
		}
...
//============================= USE_CLIST ======================================>>>
		...
		t_description= win_wstring_from_utf8(ConvertAmpersandString(GetDescriptionByIndex(nGame, GetUsechineseList())));
//===============================================================================>>>
...
void InitMainMenu(HMENU hMainMenu)
{
	...
	SetMenuItemBitmaps(hMainMenu, ID_PLAY_IPS, MF_BYCOMMAND, hFolders, hFolders);	//缘来是你 IPS
...
void InitBodyContextMenu(HMENU hBodyContextMenu)
{
	...
	_sntprintf(tmp, std::size(tmp), TEXT("%s 的属性\tAlt+D"), lpFolder->m_lptTitle);
...
static bool CommonListDialog(common_file_dialog_proc cfd, int filetype)
{
	...
	{
		of.lpstrTitle  = TEXT("输入游戏列表文件名称");
		of.lpstrFilter = TEXT("标准文本文件 (*.txt)\0*.txt\0");
		of.lpstrInitialDir = list_directory;
		of.lpstrDefExt = TEXT("txt");
	}
	else
	{
		of.lpstrTitle  = TEXT("输入过滤器名称");
		of.lpstrFilter = TEXT("过滤文件 (*.ini)\0*.ini\0");
	...
	while(!success)
	{
		if (GetSaveFileName(&of))
		{
			...
			{
				if (winui_message_box_utf8(hMain, "文件已存在，覆盖 ？", MAMEUINAME, MB_ICONQUESTION | MB_YESNO) != IDYES )
					continue;
				else
					success = true;
...
static void SaveGameListToFile(char *szFile)
{
	...
	if (f == NULL)
	{
		ErrorMessageBox("错误：无法访问文件");
		return;
	}

	// Title
	fprintf(f, "%s %s.%s", MAMEUINAME, build_version, CrLf);
	fprintf(f, "这是当前的游戏列表。%s", CrLf);
	...
	fprintf(f, "\\");
	fprintf(f, "%s>%s.%s", lpFolder->m_lpTitle, (lpFolder->m_dwFlags & F_CUSTOM) ? " (自定义文件夹)" : "", CrLf);

	// Sorting
	if (GetSortColumn() > 0)

		fprintf(f, "按 <%s>降序排序", win_utf8_from_wstring(column_names[GetSortColumn()]));
	else
		fprintf(f, "按 <%s>升序排序", win_utf8_from_wstring(column_names[-GetSortColumn()]));

	fprintf(f, ", 找到 %d 个游戏。%s", nListCount, CrLf);

	//缘来是你
    fprintf(f, "ROMs 名称\t游戏名称\t生产厂商\n");	// 导出当前游戏列表
...
//======================= 缘来是你：导出当前游戏列表 ==============>>>			
			...
			//fprintf(f, "%s%s", GetDescriptionByIndex(nGameIndex, GetUsechineseList()), "\n");	//仅游戏名称
...
//=============================================================>>>
	...
	fclose(f);
	winui_message_box_utf8(hMain, "文件保存成功。", MAMEUINAME, MB_ICONINFORMATION | MB_OK);
}
...
static void SaveROMListToFile(char *szFile)
{
	...
	fclose(f);
	winui_message_box_utf8(hMain, "文件已成功保存。", MAMEUINAME, MB_ICONINFORMATION | MB_OK);
}
...
{
    if (GetUsechineseList() && tsv_index)
    {
...
static void ExportFullXML(int mode, HWND hWndList)
{
    ...
    if (!f)
    {
        ErrorMessageBox("无法创建文件: %s", utf8_filename);
        free(utf8_filename);
        return;
    }
    ...
    char msg[256];
    snprintf(msg, sizeof(msg), "已导出 %d 个游戏到:\n%s", exported, utf8_filename);
